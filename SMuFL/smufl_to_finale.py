...
# Glyphs to generate
# (SMuFL name, Windows name, Mac name, Mac unicode index, metrics mod. values)
gen_dict = {
    'uniE01A': ('uniF03D', 'equal', 61, 0),
    'uniE022': ('uniF05F', 'underscore', 95, 'bottom'),
    ...
    'uniE0CE': ('uniF07D', 'braceright', 125, 'zero_wdth'),
    'uniE0D9': ('uniF04F', 'O', 79, 0),
    'uniE0DB': ('uniF0E2', 'quotesinglbase', 8218, 0),
    'uniE0DD': ('uniF0E1', 'periodcentered', 183, 0),
    'uniE0DE': ('uniF0B4', 'yen', 165, 0),
    'uniE0F5': ('uniF028', 'parenleft', 40, 'bottom_half'),
    'uniE0F6': ('uniF029', 'parenright', 41, 'bottom_half'),
    ...
    'uniE4A2': ('uniF02E', 'period', 46, 'center'),
    'uniE220': ('uniF021', 'exclam', 33, 'bottom'),
    'uniE221': ('uniF040', 'at', 64, 'bottom'),
    'uniE222': ('uniF0BE', 'ae', 230, 'bottom'),
    'uniE240': ('uniF06A', 'j', 106, 287),
    'uniE240.salt01': ('uniF0FB', 'ring', 730, 250),
    'uniE240.ss02': ('uniF091', 'edieresis', 235, 287),
    'uniE241': ('uniF04A', 'J', 74, -290),
    'uniE241.salt01': ('uniF0F0', 'apple', 63743, -250),
    'uniE241.ss02': ('uniF093', 'igrave', 236, -290),
    'uniE242': ('uniF072', 'r', 114, 297),
    'uniE242.ss02': ('uniF090', 'ecircumflex', 234, 389),
    'uniE243': ('uniF052', 'R', 82, -308),
    'uniE243.ss02': ('uniF092', 'iacute', 237, -421),
    'uniE250': ('uniF04B', 'K', 75, 348),
    'uniE251': ('uniF0EF', 'Ocircumflex', 212, -388),
    ...
    'uniE566': ('uniF0D9', 'Ydieresis', 376, -125),
    ...
    'uniE870': ('uniF0F7', 'tilde', 732, 'center'),
    'uniE871': ('uniF0BF', 'oslash', 248, 'center'),
    'uniE872': ('uniF084', 'Ntilde', 209, 'center'),
    ...
    'uniEAA4': ('uniF07E', 'asciitilde', 126, -125)
}
...
def unicode_index(os_name):
    ''' Returns OS dependent Unicode Index definition. '''
    if 'uni' in os_name:
        return int(os_name[3:], 16)
    else:
        return mac_ui


def genarate_glyphs(os_name):
    '''' Main process '''
    smufl_indx = f.FindGlyph(smufl_name)
    smufl_glyph = f.glyphs[smufl_indx]
    new_glyph = Glyph()
    new_glyph.name = os_name
    new_glyph.unicode = unicode_index(os_name)
    new_glyph.mark = 120
    bbox = smufl_glyph.GetBoundingRect()
    bbox_bottom = bbox.ll.y
    bbox_top = bbox.ur.y
    bbox_center = (bbox_bottom + bbox_top) / 2
    x, y = 0, 0
    if metrics_mod == 'zero_wdth':
        x = int(bbox.width - smufl_glyph.width) / 2
    elif metrics_mod == 'bottom':
        y = -int(bbox_bottom)
    elif metrics_mod == 'top':
        y = -int(bbox_top)
    elif metrics_mod == 'center':
        y = -int(bbox_center)
    elif metrics_mod == 'bottom_half':
        y = -int(bbox_center - bbox_top / 2)
    elif metrics_mod is not str(metrics_mod):
        y = int(metrics_mod)

    if f.has_key(os_name):
        print('Skipping prexisting glyph: {}'.format(os_name))
    else:
        new_glyph.components.append(Component(smufl_indx, Point(x, y)))

        # Sets metrics.
        metrics = smufl_glyph.GetMetrics()
        if metrics_mod == 'zero_wdth':
            metrics.x = bbox.width
        new_glyph.SetMetrics(metrics)
        # Appends glyphs.
        print('Appending glyph: {}'.format(os_name))
...
for smufl_name, (win_name, mac_name, mac_ui, metrics_mod) in gen_dict.iteritems():
    check_sources(smufl_name)
    if f.has_key(smufl_name):
        genarate_glyphs(mac_name)
        genarate_glyphs(win_name)
