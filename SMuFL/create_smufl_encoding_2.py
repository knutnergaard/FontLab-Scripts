...
for key, value in g.items():
    if type(value) is dict:
        values = value['codepoint'], value['description'.encode('utf-8')], key
...
try:
    ...
    with open(file_path, 'w') as f:
        ...
        for r_start, r_end, r_name in r_sorted:
            ranges = '\n\n--{}_({}-{})--'.format(r_name.replace(' ', '_').encode('utf-8'), r_start, r_end)
            f.write(ranges)
            for codepoint, g_description, g_name in g_sorted:
                g_description = g_description.encode('utf-8')
